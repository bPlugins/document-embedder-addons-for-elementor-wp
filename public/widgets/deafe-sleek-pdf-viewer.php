...
class deafe_sleek_pdf_viewer extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Sleek PDF Viewer', 'document-embedder-addons-for-elementor' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} #sleek-pdf-viewer' => 'height: {{SIZE}}{{UNIT}}',
				],
			]
		);
        $this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} #sleek-pdf-viewer' => 'max-width: {{SIZE}}{{UNIT}}',
				],
			]
		);
	...
	protected function render() {
		...
		?>
		<div class ="sleek_pdf_viewer_style">
			<?php 
			if($file_name_text !=''):?>
		...
		<?php if($final_pdf_link == ''): ?>
			<center><h3><?php esc_html_e('Paste the pdf file link from setting widget. The viewer will be worked on live site and publicly accessible pdf.','document-embedder-addons-for-elementor') ?></h3></center>
		<?php else: ?>
		<div id="sleek-pdf-viewer"></div>
        ...
        <script async type="module">
            ...
            EmbedPDF.init({
                type: 'container',
                target: document.getElementById('sleek-pdf-viewer'),
                src: '<?php echo esc_url($final_pdf_link); ?>',
            });
        </script>
	<?php endif; 
	}
